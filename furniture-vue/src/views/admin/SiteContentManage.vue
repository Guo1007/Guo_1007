<template>
  <div class="manage-page">
    ...
    <div
      class="sc-grid"
      v-if="['carousel', 'story', 'brand'].includes(activeGroup)"
    >
      <div class="sc-card" v-for="item in filteredList" :key="item.id">
        ...
        <el-form label-width="80px" size="default" class="sc-form">
          ...
          <!-- Value card icon -->
          <el-form-item
            v-if="item.sectionKey.startsWith('value_')"
            label="图标"
          >
          ...
          <el-form-item v-if="item.sectionKey !== 'brand_image'" label="标题">
            <el-input
              ...
              :placeholder="
                item.sectionKey.startsWith('value_') ? '如：天然选材' : '标题'
              "
            />
          </el-form-item>
          ...
          <el-form-item v-if="showDescFor(item.sectionKey)" label="描述">
            <el-input
              ...
              type="textarea"
              :rows="item.sectionKey === 'brand_intro' ? 6 : 2"
              placeholder="描述"
            />
          </el-form-item>
          ...
          <template v-if="item.sectionKey.startsWith('hero_')">
            <el-form-item label="小标签">
              <el-input
                v-model="extraCache[item.id].tag"
                placeholder="如：2026新品上市"
              />
            </el-form-item>
            <el-form-item label="按钮文字">
              <el-input
                v-model="extraCache[item.id].cta"
                placeholder="如：立即探索"
              />
            </el-form-item>
    ...
    <div
      class="sc-card"
      v-if="activeGroup === 'contact' && filteredList.length > 0"
    >
      <div class="sc-card-hd">
      ...
      <el-form label-width="80px" size="default" class="sc-form">
        ...
        <el-form-item label="热线备注">
          <el-input
            v-model="contactExtra.phoneNote"
            placeholder="周一至周日 9:00-21:00"
          />
        </el-form-item>
        <el-form-item label="电子邮箱">
          <el-input
            v-model="contactExtra.email"
            placeholder="service@woodspace.com"
          />
        </el-form-item>
        <el-form-item label="邮箱备注">
          <el-input
            v-model="contactExtra.emailNote"
            placeholder="24小时内回复"
          />
        </el-form-item>
        <el-form-item label="总部地址">
          <el-input
            v-model="contactExtra.address"
            type="textarea"
            :rows="2"
            placeholder="广东省深圳市..."
          />
      ...
      <div class="sc-card-actions">
        <el-button type="primary" size="small" @click="saveContact"
          >保存</el-button
        >
      </div>
    </div>
    ...
    <div class="sc-grid" v-if="activeGroup === 'label'">
      <div class="sc-card" v-for="item in filteredList" :key="item.id">
        ...
        <el-form label-width="80px" size="default" class="sc-form">
          <el-form-item
            v-if="item.contentTitle !== null && item.contentTitle !== undefined"
            label="标题"
          >
            <el-input v-model="item.contentTitle" placeholder="标题" />
          </el-form-item>
          <el-form-item
            v-if="item.contentText !== null && item.contentText !== undefined"
            label="描述"
          >
            <el-input
              v-model="item.contentText"
    ...
    <div class="sc-grid" v-if="activeGroup === 'brand'">
      <div class="sc-card" v-for="item in filteredList" :key="item.id">
        ...
        <el-form label-width="80px" size="default" class="sc-form">
          <el-form-item
            v-if="item.sectionKey === 'system_logo'"
            label="Logo图片"
          >
            <div class="image-upload-row">
              <img
      ...
      <div class="sc-card" v-for="item in filteredList" :key="item.id">
        ...
        <el-form label-width="60px" size="default" class="sc-form">
          <el-form-item label="图标">
            <el-input
              v-model="extraCache[item.id].icon"
              placeholder="如：🚚"
              style="width: 120px"
            />
          </el-form-item>
...
const showImageFor = (key) =>
  ["hero_1", "hero_2", "hero_3", "brand_image", "system_logo"].includes(key);
